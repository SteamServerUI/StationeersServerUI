...
func ServeConfigPage(w http.ResponseWriter, r *http.Request) {
	...
	data := ConfigTemplateData{
		...
		IsCleanupEnabledTrueSelected:                  isCleanupEnabledTrueSelected,
		IsCleanupEnabledFalseSelected:                 isCleanupEnabledFalseSelected,
		BackupKeepDailyFor:                            fmt.Sprintf("%d", int(config.GetBackupKeepDailyFor().Hours())),
		BackupKeepWeeklyFor:                           fmt.Sprintf("%d", int(config.GetBackupKeepWeeklyFor().Hours())),
		BackupKeepMonthlyFor:                          fmt.Sprintf("%d", int(config.GetBackupKeepMonthlyFor().Hours())),
		BackupCleanupInterval:                         fmt.Sprintf("%d", int(config.GetBackupCleanupInterval().Hours())),
		BackupWaitTime:                                fmt.Sprintf("%d", int(config.GetBackupWaitTime().Seconds())),
